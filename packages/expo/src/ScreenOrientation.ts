...
export async function addOrientationChangeListenerAsync(listener: OrientationChangeListener): Promise<EmitterSubscription> {
  ...
  const subscription = _orientationChangeEmitter.addListener(eventName, async update => {
    ...
    if (Platform.OS === 'ios'){
      ...
      [orientationLock, orientation] = await Promise.all([ExpoScreenOrientation.getOrientationLockAsync(), ExpoScreenOrientation.getOrientationAsync()])
    }
  ...
  if (Platform.OS === 'ios'){
    await ExpoScreenOrientation.addOrientationChangeListener();
  }
  return subscription;
}
...
export async function removeOrientationChangeListenersAsync(): Promise<void> {
  ...
  if (Platform.OS === 'ios'){
    ...
    await ExpoScreenOrientation.removeOrientationChangeListener();
  }
}
